...
const ApiUtils = {
  ...
  async fetchItemDetails(itemId) {
    try {
      ...
      const response = await fetch(
        `${STATE.jellyfinData.serverAddress}/Users/${STATE.jellyfinData.userId}/Items/${itemId}`,
        {
   ...
   */ async fetchItemIdsFromServer() {
    try {
      ...
      const response = await fetch(
        `${STATE.jellyfinData.serverAddress}/Items?IncludeItemTypes=Movie,Series&Recursive=true&sortBy=Random&Limit=${CONFIG.maxItems}&fields=Id&enableUserData=true&isPlayed=False&Years=${yearFilter}&Fields=ImageTags`,
        {
          headers: this.getAuthHeaders(),
        }
      );
      ...
      const items =
        data.Items.filter((item) => item.ImageTags?.Logo !== undefined) || [];
...
const SlideCreator = {
  ...
  createSlideElement(item, title) {
    ...
    const featuredContent = SlideUtils.createElement(
      ...
      title
    ); // MODIFICATION: Conditionally create plot container
    let plotContainer = null;
    if (item.Overview) {
      plotContainer = SlideUtils.createElement("div", {
    ...
    } // MODIFICATION: Create actors list

    const actorsList = this.createActorsList(item);
    ...
    slide.append(
      ...
      gradientOverlay,
      featuredContent,
      plotContainer, // Will be ignored if null
      actorsList, // Will be ignored if null
      infoContainer,
      genreElement,
      buttonContainer
    );
  ...
  /**
   * MODIFIED: Creates the rating information element only if data is available
   * @param {Object} item - Item data
   ...
   */ createRatingInfo(item) {
    ...
    const ratingContainer = SlideUtils.createElement("div", {
      className: "rating-value",
    });
    const elements = []; // IMDb Rating

    if (typeof rating === "number" && rating > 0) {
      const imdbElement = SlideUtils.createElement("div", {
        className: "rating-item imdb-rating",
      });
      const imdbLogo = SlideUtils.createElement("div", {
        className: "imdb-logo",
        innerHTML: CONFIG.IMAGE_SVG.imdbLogo,
      });
      const ratingSpan = SlideUtils.createElement(
        "span",
        {},
        rating.toFixed(1)
      );
      imdbElement.append(imdbLogo, ratingSpan);
      elements.push(imdbElement);
    } // Rotten Tomatoes Rating

    if (typeof criticRating === "number") {
      const tomatoElement = SlideUtils.createElement("div", {
        className: "rating-item tomato-rating",
      });
      const tomatoLogo = SlideUtils.createElement("div", {
        className: "tomato-logo",
        innerHTML: CONFIG.IMAGE_SVG.tomatoLogo,
      });
      const valueElement = SlideUtils.createElement(
        "span",
        {},
        `${criticRating}%`
      );
      ...
      tomatoElement.append(tomatoLogo, valueElement, criticLogo);
      elements.push(tomatoElement);
    } // Premiere Date

    const year = date ? new Date(date).getFullYear() : null;
    if (year) {
      const dateElement = SlideUtils.createElement(
        "div",
        { className: "date" },
        String(year)
      );
    ...
    if (age) {
      const ageElement = SlideUtils.createElement(
        "div",
        { className: "age-rating" },
        age
      );
      elements.push(ageElement);
    } // Runtime / Seasons
    ...
    if (runtime || season) {
      ...
      if (season === undefined) {
        const hours = Math.floor(runtime / 36000000000);
        const minutes = Math.floor((runtime % 36000000000) / 600000000);
    ...
    } // Join all available elements with separators
  ...
  /**
   * NEW: Creates a clickable list of actors
   * @param {Object} item - Item data
   * @returns {HTMLElement|null} Actors list element or null
   */
  createActorsList(item) {
    ...
    const actorsContainer = SlideUtils.createElement("div", {
      className: "actors-list",
    });
    const label = SlideUtils.createElement("strong", {}, "Starring:");
    actorsContainer.appendChild(label);

    actors.forEach((actor, index) => {
      // Create a clickable link for each actor
      const actorLink = SlideUtils.createElement(
        "a",
        {
          className: "actor-link",
          href: `#/details?id=${actor.Id}&serverId=${STATE.jellyfinData.serverId}`,
        },
        actor.Name
      );

      // Stop the click from navigating the whole slide
      actorLink.addEventListener("click", (e) => e.stopPropagation());

      actorsContainer.appendChild(actorLink);

      // Add a separator, but not after the last actor
      if (index < actors.length - 1) {
        const separator = document.createTextNode(", ");
        actorsContainer.appendChild(separator);
      }
    });
...
const SlideshowManager = {
   ...
   */ upgradeSlideImageQuality(slide) {
    ...
    images.forEach((img) => {
      const highQualityUrl = img.getAttribute("data-high-quality"); // Prevent duplicate requests if already using high quality
